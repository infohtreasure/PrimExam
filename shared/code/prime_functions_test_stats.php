<?php
//============================================================+
// File name   : prime_functions_test_stats.php
// Begin       : 2004-06-10
...
function F_getRawTestStat($test_id, $group_id=0, $user_id=0, $startdate=0, $enddate=0, $testuser_id=0, $data=array(), $pubmode=false) {
	require_once('../config/prime_config.php');
	require_once('../../shared/code/prime_functions_authorization.php');
	require_once('../../shared/code/prime_functions_test.php');
	global $db, $l;
	$test_id = intval($test_id);
...
function F_printTestStat($test_id, $group_id=0, $user_id=0, $startdate=0, $enddate=0, $testuser_id=0, $ts=array(), $display_mode=2, $pubmode=false) {
	...
	foreach ($ts['qstats']['module'] as $module) {
		...
		if ($pubmode) {
			...
			$ret .= '<td rowspan="2" valign="middle"><a href="prime_edit_module.php?module_id='.$module['id'].'" title="'.$l['t_modules_editor'].'"><strong>M'.$num_module.'</strong></a></td>'.K_NEWLINE;
		}
		...
		$ret .= '</tr>'.K_NEWLINE;
		$ret .= '<tr>';
		$ret .= '<td colspan="8" align="'.$txtdir.'" style="background-color:white;">'.F_decode_primecode($module['name']).'</td>';
		$ret .= '</tr>'.K_NEWLINE;
		if ($display_mode > 2) {
			$num_subject = 0;
			foreach ($module['subject'] as $subject) {
				...
				if ($pubmode) {
					...
					$ret .= '<td rowspan="2" valign="middle"><a href="prime_edit_subject.php?subject_id='.$subject['id'].'" title="'.$l['t_subjects_editor'].'"><strong>S'.$num_subject.'</strong></a></td>'.K_NEWLINE;
				}
				...
				$ret .= '</tr>'.K_NEWLINE;
				$ret .= '<tr>';
				$ret .= '<td colspan="8" align="'.$txtdir.'" style="background-color:white;">'.F_decode_primecode($subject['name']).'</td>';
				$ret .= '</tr>'.K_NEWLINE;
				if ($display_mode > 3) {
					$num_question = 0;
					foreach ($subject['question'] as $question) {
						...
						if ($pubmode) {
							...
							$ret .= '<td rowspan="2" valign="middle"><a href="prime_edit_question.php?question_id='.$question['id'].'" title="'.$l['t_questions_editor'].'"><strong>Q'.$num_question.'</strong></a></td>'.K_NEWLINE;
						}
						...
						$ret .= '</tr>'.K_NEWLINE;
						$ret .= '<tr>';
						$ret .= '<td colspan="8" align="'.$txtdir.'" style="background-color:white;">'.F_decode_primecode($question['description']).'</td>';
						$ret .= '</tr>'.K_NEWLINE;
						if ($display_mode > 4) {
							$num_answer = 0;
							foreach ($question['answer'] as $answer) {
								...
								if ($pubmode) {
									...
									$ret .= '<td rowspan="2" valign="middle"><a href="prime_edit_answer.php?answer_id='.$answer['id'].'" title="'.$l['t_answers_editor'].'"><strong>A'.$num_answer.'</strong></a></td>'.K_NEWLINE;
								}
								...
								$ret .= '</tr>'.K_NEWLINE;
								$ret .= '<tr>';
								$ret .= '<td colspan="8" align="'.$txtdir.'" style="background-color:white;">'.F_decode_primecode($answer['description']).'</td>';
								$ret .= '</tr>'.K_NEWLINE;
							} // end for answer
						}
					} // end for question
...
function F_printTestResultStat($data, $nextorderdir, $order_field, $filter, $pubmode=false, $stats=1) {
	...
	foreach ($data['testuser'] as $tu) {
		...
		if (!$pubmode OR F_getBoolean($tu['test']['test_report_to_users'])) {
			$ret .= '<td><a href="prime_show_result_user.php?testuser_id='.$tu['id'].'&amp;test_id='.$tu['test']['test_id'].'&amp;user_id='.$tu['user_id'].'" title="'.$l['h_view_details'].'">'.$tu['num'].'</a></td>'.K_NEWLINE;
		} else {
		...
		if ($pubmode) {
			...
			$ret .= '<td style="text-align:'.$tdalign.';"><a href="prime_edit_test.php?test_id='.$tu['test']['test_id'].'">'.$tu['test']['test_name'].'</a></td>'.K_NEWLINE;
			$ret .= '<td style="text-align:'.$tdalign.';"><a href="prime_edit_user.php?user_id='.$tu['user_id'].'">'.$tu['user_name'].'</a></td>'.K_NEWLINE;
		...
		// to add % to user marks you will add &nbsp;'.F_formatPercentage($tu['name_perc'], to each line $ret
		// % for total mark $ret .= '<td'.$passmsg.' class="numeric">'.F_formatFloat($tu['total_score']).'&nbsp;'.F_formatPercentage($tu['total_score_perc'], false).'</td>'.K_NEWLINE;
		$ret .= '<td'.$passmsg.' class="numeric">'.F_formatFloat($tu['total_score']).'</td>'.K_NEWLINE;
		if ($stats > 0) {
			$ret .= '<td class="numeric">'.$tu['right'].'</td>'.K_NEWLINE;
			$ret .= '<td class="numeric">'.$tu['wrong'].'</td>'.K_NEWLINE;
			$ret .= '<td class="numeric">'.$tu['unanswered'].'</td>'.K_NEWLINE;
			$ret .= '<td class="numeric">'.$tu['undisplayed'].'</td>'.K_NEWLINE;
			$ret .= '<td class="numeric">'.$tu['unrated'].'</td>'.K_NEWLINE;
		}
		if ($tu['locked']) {
...
function F_printUserTestStat($testuser_id) {
	...
	if ($r = F_db_query($sql, $db)) {
		...
		while ($m = F_db_fetch_array($r)) {
			...
			$ret .= F_decode_primecode($m['question_description']).K_NEWLINE;
			if (K_ENABLE_QUESTION_EXPLANATION AND !empty($m['question_explanation'])) {
				$ret .= '<br /><span class="explanation">'.$l['w_explanation'].':</span><br />'.F_decode_primecode($m['question_explanation']).''.K_NEWLINE;
			}
			if ($m['question_type'] == 3) {
				// TEXT
				$ret .= '<ul class="answer"><li>'.K_NEWLINE;
				$ret .= F_decode_primecode($m['testlog_answer_text']);
				$ret .= '&nbsp;</li></ul>'.K_NEWLINE;
			} else {
				...
				if ($ra = F_db_query($sqla, $db)) {
					while ($ma = F_db_fetch_array($ra)) {
						...
						$ret .= ' ';
						$ret .= F_decode_primecode($ma['answer_description']);
						if (K_ENABLE_ANSWER_EXPLANATION AND !empty($ma['answer_explanation'])) {
							$ret .= '<br /><span class="explanation">'.$l['w_explanation'].':</span><br />'.F_decode_primecode($ma['answer_explanation']).''.K_NEWLINE;
						}
						$ret .= '</li>'.K_NEWLINE;
					}
				} else {
			...
			if (isset($m['testlog_comment']) AND (!empty($m['testlog_comment']))) {
				...
				$ret .= F_decode_primecode($m['testlog_comment']);
				$ret .= '&nbsp;</li></ul>'.K_NEWLINE;
			}
			$ret .= '<br /><br />'.K_NEWLINE;
			$ret .= '</li>'.K_NEWLINE;
