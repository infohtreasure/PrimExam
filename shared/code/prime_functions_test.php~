<?php
//============================================================+
// File name   : prime_functions_test.php
// Begin       : 2004-05-28
				...
				if (($test_status >= 4) AND F_getBoolean($m['test_results_to_users'])) {
					...
					if (isset($usrtestdata['user_score']) AND (strlen(''.$usrtestdata['user_score']) > 0)) {
						if ($usrtestdata['test_max_score'] > 0) {
							$str .= '<a href="prime_show_result_user.php?testuser_id='.$testuser_id.'&amp;test_id='.$m['test_id'].'" title="'.$l['h_result'].'">'.$usrtestdata['user_score'].' / '.$usrtestdata['test_max_score'].' ('.round(100 * $usrtestdata['user_score'] / $usrtestdata['test_max_score']).'%)'.$passmsg.'</a>';
						} else {
							$str .= '<a href="prime_show_result_user.php?testuser_id='.$testuser_id.'&amp;test_id='.$m['test_id'].'" title="'.$l['h_result'].'">'.$usrtestdata['user_score'].$passmsg.'</a>';
						}
					} else {
						$str .= '&nbsp;';
					}
					...
					switch ($test_status) {
						case 0: { // 0 = the test generation process is started but not completed
							...
							if (K_DISPLAY_TEST_DESCRIPTION OR !empty($m['test_password'])) {
								...
								$str .= 'prime_test_start.php';
							} else {
								// directly execute test
								$str .= 'prime_test_execute.php';
							}
						...
						case 3: { // 3 = all questions have been answered
							// continue test
							$str .= '<a href="prime_test_execute.php?testid='.$m['test_id'].'" title="'.$l['h_continue'].'" class="xmlbutton">'.$l['w_continue'].'</a>';
							break;
						}
...
function F_testInfoLink($test_id, $link_name='') {
	...
	global $db, $l;
	$str = '';
	$onclickinfo = 'infoTestWindow=window.open(\'prime_popup_test_info.php?testid='.$test_id.'\'';
	...
	$str .= '<a href="prime_popup_test_info.php?testid='.$test_id.'" onclick="'.$onclickinfo.'" title="'.$l['m_new_window_link'].'">';
...
function F_printTestInfo($test_id, $showip=false) {
	...
	if ($r = F_db_query($sql, $db)) {
		if ($m = F_db_fetch_array($r)) {
			...
			$str .= '<div class="primecontentbox">'.F_decode_primecode($m['test_description']).'<br /><br /></div>'.K_NEWLINE;
			$str .= '<div class="primeformbox">'.K_NEWLINE;
...
/**
 * Add specified answers on prime_tests_logs_answer table.
...
/**
 * Returns the ID of the prime_tests_users table corresponding to a complete test of $test_id type.
 * @param $test_id (int) test ID
...
/**
 * Creates a new prime_tests_logs table entry and returns inserted ID.
 * @param $testuser_id (int) ID of prime_tests_users
 * @param $question_id (int) question ID
...
function F_createTest($test_id, $user_id) {
	...
	$test_id = intval($test_id);
	$user_id = intval($user_id);
	$firsttest = 0; // id of the first test of this type
	// get test data
...
function F_questionForm($test_id, $testlog_id, $formname) {
	...
	if ($r = F_db_query($sql, $db)) {
		if ($m = F_db_fetch_array($r)) {
			...
			$str .= '<div class="primecontentbox">'.K_NEWLINE;
			//fieldset
			//$str .= '<legend>';
			//$str .= $l['w_question'];
			...
			$str .= F_decode_primecode($m['question_description']).K_NEWLINE;
			if ($m['question_type'] == 3) {
				$str .= '</label>';
			}
					...
					while ($ma = F_db_fetch_array($ra)) {
						...
						switch ($m['question_type']) {
							case 1: {
								...
								$str .= F_decode_primecode($ma['answer_description']);
								$str .= '</label>';
...
function F_questionsMenu($testdata, $testuser_id, $testlog_id=0, $disable=false) {
	...
	if ($r = F_db_query($sql, $db)) {
		...
		while ($m = F_db_fetch_array($r)) {
			...
			if ($m['testlog_id'] != $testlog_id) {
				$str .= '<li>';
				$str .= '<input type="submit" name="jumpquestion_'.$m['testlog_id'].'" id="jumpquestion_'.$m['testlog_id'].'" value="&gt;" title="" /> ';
				if ($testlog_id_last == $testlog_id) {
					$testlog_id_next = $m['testlog_id'];
				}
			} else {
				$str .= '<li class="selected">';
				$str .= '<input type="button" name="jumpquestion_'.$m['testlog_id'].'" id="jumpquestion_'.$m['testlog_id'].'" value="&gt;" title="" disabled="disabled"/> ';
			...
			// $str .= F_primecodeToLine($m['question_description']);
			$str .= '</li>'.K_NEWLINE;
		}
	} else {
		F_display_db_error();
	...
	if (F_getBoolean($testdata['test_menu_enabled']) AND (!$disable)) {
		...
		$rstr .= '<div class="menuecontentbox">'.K_NEWLINE; //fieldset
		//$rstr .= '<legend>';
		//$rstr .= $l['w_questions'];
