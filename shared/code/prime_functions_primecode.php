<?php
//============================================================+
// File name   : prime_functions_primecode.php
// Begin       : 2002-01-09
// Last Update : 2013-12-24
//
// Description : Functions to translate primexam code into XHTML.
//               The primexam code is compatible to the common BBCode.
...
/**
 * @file
 * Functions to translate primexam proprietary code into XHTML.
 * The primexam code is compatible to the common BBCode.
...
/**
 * Returns XHTML code from text marked-up with primexam Code Tags
 * @param $text_to_decode (string) text to convert
 * @return string XHTML code
 */
function F_decode_primecode($text_to_decode) {
	...
	// --- convert some BBCode to primeCode: ---
	...
	$replacement[++$i] = '<a class="primecode" href="\1">\1</a>';

	// [url=http://www.domain.com]linkname[/url]
	$pattern[++$i] = "#\[url=(.*?)\](.*?)\[/url\]#si";
	$replacement[++$i] = '<a class="primecode" href="\1">\2</a>';
	...
	$replacement[++$i] = '<div class="primecodepre">\1</div>';

	// [small] and [/small] for small text
	$pattern[++$i] = "#\[small\](.*?)\[/small\]#si";
	$replacement[++$i] = '<small class="primecode">\1</small>';

	// [b] and [/b] for bolding text.
	$pattern[++$i] = "#\[b\](.*?)\[/b\]#si";
	$replacement[++$i] = '<strong class="primecode">\1</strong>';

	// [i] and [/i] for italicizing text.
	$pattern[++$i] = "#\[i\](.*?)\[/i\]#si";
	$replacement[++$i] = '<em class="primecode">\1</em>';
	...
	$replacement[++$i] = '<sub class="primecode">\1</sub>';

	// [sup] and [/sup] for superscript text.
	$pattern[++$i] = "#\[sup\](.*?)\[/sup\]#si";
	$replacement[++$i] = '<sup class="primecode">\1</sup>';

	// [ulist] and [/ulist] unordered list
	$pattern[++$i] = "#\[ulist\](.*?)\[/ulist\]#si";
	$replacement[++$i] = '<ul class="primecode">\1</ul>';

	// [olist] and [/olist] ordered list.
	$pattern[++$i] = "#\[olist\](.*?)\[/olist\]#si";
	$replacement[++$i] = '<ol class="primecode">\1</ol>';

	// [olist=1] and [/olist] ordered list.
	$pattern[++$i] = "#\[olist=1\](.*?)\[/olist\]#si";
	$replacement[++$i] = '<ol class="primecode" style="list-style-type:arabic-numbers">\1</ol>';

	// [olist=a] and [/olist] ordered list.
	$pattern[++$i] = "#\[olist=a\](.*?)\[/olist\]#si";
	$replacement[++$i] = '<ol class="primecode" style="list-style-type:lower-alpha">\1</ol>';

	// [li] list items [/li]
	$pattern[++$i] = "#\[li\](.*?)\[/li\]#si";
	$replacement[++$i] = '<li class="primecode">\1</li>';
...
function F_latex_callback($matches) {
	require_once('../../shared/config/prime_latex.php');
	...
	if ($imgurl === false) {
		...
		$imsize = @getimagesize($imgpath.'.'.K_LATEX_IMG_FORMAT);
		list($w, $h) = $imsize;
		$newtext = '<img src="'.$imgurl.'" alt="'.$alt_latex.'" class="primecode" width="'.round($w / $dr).'" height="'.round($h / $dr).'" />';
	}
	return $newtext;
}
...
function F_objects_callback($matches) {
	...
	if (isset($matches[5]) AND (!empty($matches[5]))) {
		$alt = F_primecodeToTitle($matches[5]);
	}
...
function F_objects_replacement($name, $extension, $width=0, $height=0, $alt='', &$maxwidth=0, &$maxheight=0) {
	...
	switch($extension) {
		...
		default: {
			include('../../shared/config/prime_mime.php');
...
/**
 * Returns specified string without primecode mark-up tags
 * @param $str (string) text to process
 * @return string without primecode markup tags
 */
function F_remove_primecode($str) {
...
/**
 * Converts primecode text to a single XHTML string removing some objects.
 * @param $str (string) text to process
 * return string
 */
function F_primecodeToLine($str) {
	$str = preg_replace("'\[object\](.*?)\[/object([^\]]*?)\]'si", '[OBJ]',  $str);
	$str = preg_replace("'\[img([^\]]*?)\](.*?)\[/img\]'si", '[IMG]',  $str);
	$str = preg_replace("'\[code\](.*?)\[/code\]'si", '\1',  $str);
	$str = preg_replace("'\[li\](.*?)\[/li\]'si", ' * \1',  $str);
...
/**
 * Converts primecode text to simple string for XHTML title attribute.
 * @param $str (string) text to process
 * return string
 */
function F_primecodeToTitle($str) {
	require_once('../config/prime_config.php');
	global $l;
	$str = F_remove_primecode($str);
	$str = F_compact_string($str);
