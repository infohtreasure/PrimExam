...
class TCPDF {
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$cpadding = $margin / 10;
		$this->seprimellPaddings($cpadding, 0, $cpadding, 0);
		// cell margins
		$this->seprimellMargins(0, 0, 0, 0);
		// line width (0.2 mm)
	 ...
	 * @public
	 */
	public function geprimellHeight($fontsize, $padding=TRUE) {
	...
	public function resetLastH() {
		$this->lasth = $this->geprimellHeight($this->FontSize);
	}
	 ...
	 * @public
	 * @since 2.1.000 (2008-01-09)
	 * @see geprimellPaddings(), seprimellPaddings()
	 */
	public function SeprimellPadding($pad) {
		if ($pad >= 0) {
	 ...
	 * @public
	 * @since 5.9.000 (2010-10-03)
	 * @see geprimellPaddings(), SeprimellPadding()
	 */
	public function seprimellPaddings($left='', $top='', $right='', $bottom='') {
		if (($left !== '') AND ($left >= 0)) {
	 ...
	 * @public
	 * @since 5.9.000 (2010-10-03)
	 * @see seprimellPaddings(), SeprimellPadding()
	 */
	public function geprimellPaddings() {
	 ...
	 * @public
	 * @since 5.9.000 (2010-10-03)
	 * @see geprimellMargins()
	 */
	public function seprimellMargins($left='', $top='', $right='', $bottom='') {
		if (($left !== '') AND ($left >= 0)) {
	 ...
	 * @public
	 * @since 5.9.000 (2010-10-03)
	 * @see seprimellMargins()
	 */
	public function geprimellMargins() {
	 ...
	 * @since 5.9.000 (2010-10-03)
	 */
	protected function adjusprimellPadding($brd=0) {
		if (empty($brd)) {
			return;
		}
		if (is_string($brd)) {
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			$cell_height = $this->geprimellHeight($headerfont[2] / $this->k);
	...
	public function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$prev_cell_padding = $this->cell_padding;
		$this->adjusprimellPadding($border);
		if (!$ignore_min_height) {
			$min_cell_height = $this->geprimellHeight($this->FontSize);
		...
		if ($this->txtshadow['enabled']) {
			...
			if ($this->state == 2) {
				$this->_out($this->geprimellCode($w, $h, $txt, $border, $ln, $align, $fill, $link, $stretch, true, $calign, $valign));
			}
			//restore data
			$this->x = $x;
			$this->y = $y;
	 ...
	 * @see Cell()
	 */
	protected function geprimellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		// draw borders
		$s .= $this->geprimellBorder($x, $y, $w, $h, $border);
		if ($txt != '') {
			$txt2 = $txt;
			if ($this->isunicode) {
	 ...
	 */
	protected function geprimellBorder($x, $y, $w, $h, $brd) {
	 ...
	 * @param $fiprimell (boolean) if true attempt to fit all the text within the cell by reducing the font size (do not work in HTML mode). $maxh must be greater than 0 and wqual to $h.
	 * @return int Return the number of cells or 1 for html mode.
	 * @public
	 * @since 1.3
	 * @see SetFont(), SetDrawColor(), SetFillColor(), SetTextColor(), SetLineWidth(), Cell(), Write(), SetAutoPageBreak()
	 */
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fiprimell=false) {
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		...
		if ($ishtml) { // ******* Write HTML text
			...
			if ($maxh > 0) {
				...
				if ($fiprimell AND ($text_height > $maxh) AND ($this->FontSizePt > 1)) {
		...
		for ($page = $startpage; $page <= $endpage; ++$page) { // for each page
			...
			if ($startpage == $endpage) {
				...
				for ($column = $startcolumn; $column <= $endcolumn; ++$column) { // for each column
					...
					$ccode .= $this->geprimellCode($w, $h, '', $cborder, 1, '', $fill, '', 0, true)."\n";
				} // end for each column
	 ...
	 * @author Alexander Escalona Fern\E1ndez, Ifeoluwa Opeyemi O
	 * @public
	 * @since 4.5.011
	 */
	 ...
	 * @param $border (mixed) Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 * @return float Return the minimal height needed for multicell method for printing the $txt param.
	 * @author Ifeoluwa Opeyemi O, Alexander Escalona Fern\E1ndez
	 * @public
	 */
	public function getStringHeight($w, $txt, $reseth=false, $autopadding=true, $cellpadding='', $border=0) {
		...
		$height = $this->geprimellHeight(($lines * $this->FontSize), $autopadding);
		$this->cell_padding = $prev_cell_padding;
		$this->lasth = $prev_lasth;
		return $height;
	}
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		// minimum row height
		$row_height = max($h, $this->geprimellHeight($this->FontSize));
	 ...
	 * @param $font (array) font data
	 * @protected
	 * @author Andrew Whitehead, Ifeoluwa Opeyemi O, Yukihiro Nakadaira
	 ...
	 * @author Piotr Galecki, Ifeoluwa Opeyemi O, Andy Meier
	 ...
	 * @public
	 * @author Christian Deligant, Ifeoluwa Opeyemi O
	 * @since 5.9.097 (2011-06-23)
	 */
	 ...
	 * @protected
	 * @author Johannes G\FCntert, Ifeoluwa Opeyemi O
	 ...
	 * @protected
	 * @author Olivier Plathey, Ifeoluwa Opeyemi O
	 * @since 2.1.002 (2008-02-12)
	 */
	 ...
	 * @protected
	 * @author Denis Van Nuffelen, Ifeoluwa Opeyemi O
	 * @since 2.1.002 (2008-02-12)
	 */
	 ...
	 * @since 3.0.014 (2008-06-04)
	 */
	public function seprimellHeightRatio($h) {
	 ...
	 * @since 4.0.012 (2008-07-24)
	 */
	public function geprimellHeightRatio() {
	 ...
	 * @author Andreas W\FCrmser, Ifeoluwa Opeyemi O
	 * @since 3.1.000 (2008-06-09)
	 * @public
	 */
	 ...
	 * @author Maxime Delorme, Ifeoluwa Opeyemi O
	 * @since 3.1.000 (2008-06-09)
	 * @public
	 */
	 ...
	 * @author Valentin Schmidt, Ifeoluwa Opeyemi O
	 * @since 3.1.000 (2008-06-09)
	 * @public
	 */
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		...
		$text_height = $this->geprimellHeight($fontsize / $this->k);
		// height
		if (($h === '') OR ($h <= 0)) {
			// set default height
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				if ((!$this->newline) AND ($dom[$key]['value'] == 'img') AND (isset($dom[$key]['height'])) AND ($dom[$key]['height'] > 0)) {
					...
					if (!$autolinebreak) {
						...
						$this->y += ($this->geprimellHeight($curfontsize / $this->k) - ($curfontdescent * $this->cell_height_ratio) - $imgh);
						$minstartliney = min($this->y, $minstartliney);
						$maxbottomliney = ($startliney + $this->geprimellHeight($curfontsize / $this->k));
					}
							...
							if (!$dom[$key]['block']) {
								...
								if (($dom[$key]['value'] != 'sup') AND ($dom[$key]['value'] != 'sub')) {
									...
									if (isset($line_align_data) AND (($line_align_data[0] == ($key - 1)) OR (($line_align_data[0] == ($key - 2)) AND (isset($dom[($key - 1)])) AND (preg_match('/^([\s]+)$/', $dom[($key - 1)]['value']) > 0)))) {
										...
										$maxbottomliney = max(($this->y + $this->geprimellHeight($fontsize / $this->k)), $line_align_data[2]);
									} else {
										$minstartliney = min($this->y, $minstartliney);
										$maxbottomliney = max(($this->y + $this->geprimellHeight($fontsize / $this->k)), $maxbottomliney);
									}
			...
			if ($this->newline AND (strlen($dom[$key]['value']) > 0) AND ($dom[$key]['value'] != 'td') AND ($dom[$key]['value'] != 'th')) {
				...
				if ($dom[$dom[$key]['parent']]['value'] == 'sup') {
					...
					$maxbottomliney = ($this->y + $this->geprimellHeight($fontsize / $this->k));
				}
				$startlinepage = $this->page;
			...
			if ($dom[$key]['tag']) {
				...
				if (!TCPDF_STATIC::empty_string($this->lispacer) AND ($this->lispacer != '^')) {
					...
					$this->resetLastH();
					$minstartliney = $this->y;
					$maxbottomliney = ($startliney + $this->geprimellHeight($this->FontSize));
					...
					if (is_numeric($pfontsize) AND ($pfontsize > 0) AND is_numeric($curfontsize) AND ($curfontsize > 0) AND ($pfontsize != $curfontsize)) {
						...
						$this->y += ($this->geprimellHeight(($pfontsize - $curfontsize) / $this->k) + $pfontascent - $curfontascent - $pfontdescent + $curfontdescent) / 2;
						$minstartliney = min($this->y, $minstartliney);
						$maxbottomliney = max(($this->y + $this->geprimellHeight($pfontsize / $this->k)), $maxbottomliney);
					}
				}
				// text
				$this->htmlvspace = 0;
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		if ($tag['block']) {
			...
			if (isset($this->tagvspaces[$tag['value']][0]['h']) AND ($this->tagvspaces[$tag['value']][0]['h'] >= 0)) {
				...
				$cur_h = $this->geprimellHeight($tag['fontsize'] / $this->k);
			} else {
				$cur_h = $this->geprimellHeight($this->FontSize);
			}
			...
			if (($this->htmlvspace <= 0) AND ($on > 0)) {
				if (isset($parent['fontsize'])) {
					...
					$hbz = $this->geprimellHeight($this->FontSize);
				}
			}
			...
			if (isset($this->tagvspaces[$tag['value']][1]['h']) AND ($this->tagvspaces[$tag['value']][1]['h'] >= 0)) {
				...
				$pre_h = $this->geprimellHeight($parent['fontsize'] / $this->k);
			} else {
				$pre_h = $this->geprimellHeight($this->FontSize);
			}
		...
		switch($tag['value']) {
			...
			case 'img': {
				if (!empty($tag['attribute']['src'])) {
					...
					switch($align) {
						...
						case 'M': {
							$this->y = (($this->img_rb_y + $prevy - ($this->geprimellHeight($tag['fontsize'] / $this->k))) / 2);
							break;
						}
						case 'B': {
							$this->y = $this->img_rb_y - ($this->geprimellHeight($tag['fontsize'] / $this->k) - ($this->getFontDescent($tag['fontname'], $tag['fontstyle'], $tag['fontsize']) * $this->cell_height_ratio));
							break;
						}
					}
				}
			...
			case 'input': {
				...
				$h = $this->geprimellHeight($this->FontSize);
			...
			case 'textarea': {
				...
				if (isset($tag['attribute']['rows']) AND !TCPDF_STATIC::empty_string($tag['attribute']['rows'])) {
					$h = intval($tag['attribute']['rows']) * $this->geprimellHeight($this->FontSize);
				} else {
					$h = 10;
				}
				$prop['multiline'] = 'true';
	...
	public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		foreach ($this->outlines as $key => $outline) {
			...
			// check for page break
			$this->checkPageBreak(2 * $this->geprimellHeight($this->FontSize));
