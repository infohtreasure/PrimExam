<?php
//============================================================+
// File name   : prime_test_results.php
// Begin       : 2004-06-10
...
echo getFormDescriptionLine($l['w_comment'].':', $l['h_testcomment'], F_decode_primecode($usrtestdata['comment']));
	...
	if (isset($testuser_id) AND (!empty($testuser_id))) {
		...
		if($r = F_db_query($sql, $db)) {
			...
			while($m = F_db_fetch_array($r)) {
				...
				echo F_decode_primecode($m['question_description']).K_NEWLINE;
				if (K_ENABLE_QUESTION_EXPLANATION AND !empty($m['question_explanation'])) {
					echo '<br /><span class="explanation">'.$l['w_explanation'].':</span><br />'.F_decode_primecode($m['question_explanation']).''.K_NEWLINE;
				}
				if ($m['question_type'] == 3) {
					// TEXT
					echo '<ul class="answer"><li>'.K_NEWLINE;
					echo F_decode_primecode($m['testlog_answer_text']);
					...
					if($ra = F_db_query($sqla, $db)) {
						while($ma = F_db_fetch_array($ra)) {
							...
							echo ' ';
							echo F_decode_primecode($ma['answer_description']);
							if (K_ENABLE_ANSWER_EXPLANATION AND !empty($ma['answer_explanation'])) {
								echo '<br /><span class="explanation">'.$l['w_explanation'].':</span><br />'.F_decode_primecode($ma['answer_explanation']).''.K_NEWLINE;
							}
							echo '</li>'.K_NEWLINE;
						}
					} else {
				...
				if (isset($m['testlog_comment']) AND (!empty($m['testlog_comment']))) {
					...
					echo F_decode_primecode($m['testlog_comment']);
					echo '&nbsp;</li></ul>'.K_NEWLINE;
				}
