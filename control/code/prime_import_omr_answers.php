<?php
//============================================================+
// File name   : prime_import_omr_answers.php
// Begin       : 2011-05-20
...
if (isset($menu_mode) AND ($menu_mode == 'upload') AND ($user_id > 0) AND !empty($_FILES)) {
	...
	if ($omr_testdata === false) {
		...
		if (F_importOMRTestData($user_id, $date, $omr_testdata, $omr_answers, $overwrite)) {
			F_print_error('MESSAGE', $l['m_import_ok'].': <a href="prime_show_result_user.php?testuser_id=32&test_id='.$omr_testdata[0].'&user_id='.$user_id.'" title="'.$l['t_result_user'].'" style="text-decoration:underline;color:#0000ff;">'.$l['w_results'].'</a>');
		} else {
