...
$out_filename = 'primexam_users_'.date('YmdHis');
...
function F_xml_export_users() {
	...
	$xml .= '<primexamusers version="'.K_primexam_VERSION.'">'.K_NEWLINE;
	$xml .=  K_TAB.'<header';
	...
	if($ra = F_db_query($sqla, $db)) {
		while($ma = F_db_fetch_array($ra)) {
			...
			$xml .= K_TAB.K_TAB.K_TAB.K_TAB.'<passport>';
			$xml .= F_text_to_xml($ma['user_passport']);
			$xml .= '</passport>'.K_NEWLINE;
