<?php
//============================================================+
// File name   : prime_functions_user_select.php
// Begin       : 2001-09-13
...
function F_show_select_user($order_field, $orderdir, $firstrow, $rowsperpage, $group_id=0, $andwhere='', $searchterms='') {
	...
	if (empty($order_field) OR (!in_array($order_field, array('user_id', 'user_name', 'user_password', 'user_email', 'user_regdate', 'user_ip', 'user_firstname', 'user_lastname', 'user_birthdate', 'user_birthplace', 'user_regnumber', 'user_passport', 'user_level', 'user_verifycode')))) {
	...
	if ($r = F_db_query($sql, $db)) {
		if ($m = F_db_fetch_array($r)) {
			...
			echo '<th title="'.$l['t_all_results_user'].'">'.$l['w_exams'].'</th>'.K_NEWLINE;
			echo '</tr>'.K_NEWLINE;
			$itemcount = 0;
			do {
				$itemcount++;
				...
				echo ' />';
				echo '</td>'.K_NEWLINE;
				echo '<td style="text-align:'.$txtalign.';">&nbsp;<a href="prime_edit_user.php?user_id='.$m['user_id'].'" title="'.$l['w_edit'].'">'.htmlspecialchars($m['user_name'], ENT_NOQUOTES, $l['a_meta_charset']).'</a></td>'.K_NEWLINE;
				...
				echo '<td><a href="prime_show_result_allusers.php?user_id='.$m['user_id'].'" class="xmlbutton" title="'.$l['t_all_results_user'].'">...</a></td>'.K_NEWLINE;
			...
			echo '<br />';
			echo '<a href="prime_xml_users.php" class="xmlbutton" title="'.$l['h_xml_export'].'">XML</a> ';
			echo '<a href="prime_xml_users.php?format=JSON" class="xmlbutton" title="JSON">JSON</a> ';
			echo '<a href="prime_tsv_users.php" class="xmlbutton" title="'.$l['h_tsv_export'].'">TSV</a>';
			echo '</div>'.K_NEWLINE;
