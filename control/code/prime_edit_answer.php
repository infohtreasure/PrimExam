<?php
//============================================================+
// File name   : prime_edit_answer.php
// Begin       : 2004-04-27
...
if (($subject_module_id <= 0) OR ($question_subject_id <= 0) OR ($answer_question_id <= 0)) {
	...
	echo '<p><a href="prime_edit_question.php" title="'.$l['t_questions_editor'].'" class="xmlbutton">&lt; '.$l['t_questions_editor'].'</a></p>'.K_NEWLINE;
...
if($r = F_db_query($sql, $db)) {
	...
	while($m = F_db_fetch_array($r)) {
		...
		echo ' '.htmlspecialchars(F_substr_utf8(F_remove_primecode($m['answer_description']), 0, K_SELECT_SUBSTRING), ENT_NOQUOTES, $l['a_meta_charset']).'</option>'.K_NEWLINE;
		$countitem++;
	}
	if ($countitem == 1) {
		echo '<option value="0">&nbsp;</option>'.K_NEWLINE;
...
echo '<br />'.K_NEWLINE;
echo primecodeEditorTagButtons('form_answereditor', 'answer_description');
echo '</span>'.K_NEWLINE;
echo '</div>'.K_NEWLINE;
...
if (K_ENABLE_ANSWER_EXPLANATION) {
	...
	echo '<br />'.K_NEWLINE;
	echo primecodeEditorTagButtons('form_answereditor', 'answer_explanation');
	echo '</span>'.K_NEWLINE;
	echo '</div>'.K_NEWLINE;
}
...
if (isset($answer_question_id) AND ($answer_question_id > 0)) {
	echo '<a href="prime_edit_question.php?subject_module_id='.$subject_module_id.'&amp;question_subject_id='.$question_subject_id.'&amp;question_id='.$answer_question_id.'" title="'.$l['t_questions_editor'].'" class="xmlbutton">&lt; '.$l['t_questions_editor'].'</a>';
}
...
echo F_decode_primecode($answer_description);
