<?php
//============================================================+
// File name   : prime_import_questions.php
// Begin       : 2006-03-12
...
if (isset($menu_mode) AND ($menu_mode == 'upload')) {
	if ($_FILES['userfile']['name']) {
		...
		if ($uploadedfile !== false) {
			...
			switch($type) {
				case 1: {
					// standard primexam XML format
					require_once('../code/prime_class_import_xml.php');
					$qimp = new XMLQuestionImporter(K_PATH_CACHE.$uploadedfile);
					break;
				}
				case 2: {
					// standard primexam TSV format
					$qimp = F_TSVQuestionImporter(K_PATH_CACHE.$uploadedfile);
					break;
				}
				case 3: {
					// Custom primexam XML format
					require_once('../code/prime_import_custom.php');
...
echo '<input type="radio" name="type" id="type_xml" value="1" title="primexam XML Format"';
if ($type == 1) {echo ' checked="checked"';}
echo ' />';
echo '<label for="type_xml">primexam XML</label><br />'.K_NEWLINE;

echo '<input type="radio" name="type" id="type_tsv" value="2" title="primexam TSV Format"'.K_NEWLINE;
if ($type == 2) {echo ' checked="checked"';}
echo ' />';
echo '<label for="type_tsv">primexam TSV</label>'.K_NEWLINE;
 ...
 * The format of TSV is the same obtained by exporting data from primexam interface.
