<?php
//============================================================+
// File name   : prime_pdf_testgen.php
// Begin       : 2004-06-13
...
$header_backlink = K_PATH_URL.'control/code/prime_edit_test.php?test_id='.$test_id;
$pdf->setprimexamBackLink($header_backlink);
...
for ($item = 1; $item <= $test_num; $item++) {
	...
	// test description
	$pdf->writeHTMLCell(0, ($boxEndY - $infoStartY + 4), (PDF_MARGIN_LEFT + ($info_cell_width * 2)), $infoStartY - 2, F_decode_primecode($testdata['test_description']), 1, 1);
	...
	*/
	...
	if ($r = F_db_query($sql, $db)) {
		...
		foreach ($questions_data as $key => $q) {
			...
			while ($print_block > 0) {
				...
				$pdf->writeHTMLCell(0, $data_cell_height, (PDF_MARGIN_LEFT + (2 * $data_cell_width_third)), $pdf->GetY(), F_decode_primecode($q['description']), 1, 1);
				// ------------------------------
					...
					if($ra = F_db_query($sqla, $db)) {
						if($ma = F_db_fetch_array($ra)) {
							...
							while ($print_block > 0) {
								...
								$pdf->writeHTMLCell(0, $data_cell_height, (PDF_MARGIN_LEFT + $data_cell_width + $data_cell_width_third), $pdf->GetY(), F_decode_primecode($ma['answer_description']), 1, 1);
								// ------------------------------
	...
	$pdf->SeprimellPadding($line_width);
...
$pdf->Output('primexam_test_'.$test_id.'_'.date('YmdHis').'.pdf', 'D');
