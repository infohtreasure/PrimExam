...
switch($menu_mode) { // process submitted data
	...
	case 'update':{ // Update user
		...
		if ($formstatus = F_check_form_fields()) {
			...
			if (isset($user_passport) AND (strlen($user_passport) > 0) AND (!F_check_unique(K_TABLE_USERS, 'user_passport=\''.F_escape_sql($db, $user_passport).'\'', 'user_id', $user_id))) {
			...
			$sql = 'UPDATE '.K_TABLE_USERS.' SET
				...
				user_level='.F_empty_to_null($user_level).'
...
echo getFormRowTextInput('newpassword', $l['w_password'], $l['h_password'], ' ('.$l['d_password_lenght'].')', '', '^([a-zA-Z0-9]{4,32})$', 255, false, false, true);
...
//echo getFormRowTextInput('user_passport', $l['w_fiscal_code'], $l['h_fiscal_code'], '', $user_passport, '', 255, false, false, false);
...
// to enable OTPkep remove the // tag
