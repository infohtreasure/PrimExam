<?php
//============================================================+
// File name   : prime_functions_filemanager.php
// Begin       : 2010-09-20
// Last Update : 2013-04-12
//
// Description : Functions for primexam file manager.
//
...
/**
 * @file
 * Functions for primexam file manager.
...
function F_getFileInfo($file) {
	...
	$info['primefile'] = substr($file, strlen(K_PATH_CACHE));
	$info['primename'] = substr($info['primefile'], 0, -(strlen($info['extension']) + 1));
	return $info;
}
	...
	foreach ($data['files'] as $file) {
		...
		if (isset($info['extension']) AND in_array(strtolower($info['extension']), $allowed_extensions) AND (substr($info['basename'],0,6) != 'latex_')) {
			...
			if (in_array(strtolower($info['extension']), $imgformats)) {
				$w = 150;
				$h = 150;
				$imgicon = F_objects_replacement($info['primename'], $info['extension'], 0, 0, $l['w_preview'], $w, $h);
			} else {
