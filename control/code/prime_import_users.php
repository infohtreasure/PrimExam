<?php
//============================================================+
// File name   : prime_import_users.php
// Begin       : 2006-03-17
 ...
 * @package com.htreasure.primexam.admin
 * @author Ifeoluwa Opeyemi O [www.htreasure.com]
 * @version 1.0.000
 */
class XMLUserImporter {
				...
				if (!empty($this->user_data['user_name'])) {
					...
					$sql = 'SELECT user_id,user_level
						...
						WHERE user_name=\''.$this->user_data['user_name'].'\'
							...
							OR user_passport=\''.$this->user_data['user_passport'].'\'
						LIMIT 1';
					if ($r = F_db_query($sql, $db)) {
						if ($m = F_db_fetch_array($r)) {
							...
							if (($_SESSION['session_user_level'] >= K_AUTH_ADMINISTRATOR) OR ($_SESSION['session_user_level'] > $m['user_level'])) {
								...
								$sqlu .= '
									...
									user_passport='.F_empty_to_null($this->user_data['user_passport']).',
							...
							$sqlu = 'INSERT INTO '.K_TABLE_USERS.' (
								...
								'.F_empty_to_null($this->user_data['user_passport']).',
	...
	while (list($item, $rowdata) = each($tsvrows)) {
		...
		$sql = 'SELECT user_id,user_level
			...
			WHERE user_name=\''.F_escape_sql($db, $userdata[1]).'\'
				...
				OR user_passport='.F_empty_to_null($userdata[11]).'
			LIMIT 1';
		if ($r = F_db_query($sql, $db)) {
			if ($m = F_db_fetch_array($r)) {
				// the user has been already added
