<?php
//============================================================+
// File name   : prime_show_all_questions.php
// Begin       : 2005-07-06
...
require_once('prime_functions_questions.php');
...
if (isset($subject_id) AND ($subject_id > 0)) {
	$pdflink = 'prime_pdf_all_questions.php';
	...
	$xmllink = 'prime_xml_questions.php';
	...
	$tsvlink = 'prime_tsv_questions.php';
...
function F_show_select_questions($wherequery, $subject_module_id, $subject_id, $order_field, $orderdir, $firstrow, $rowsperpage, $hide_answers=false) {
	...
	if ($r = F_db_query($sql, $db)) {
		...
		while($m = F_db_fetch_array($r)) {
			...
			$questlist .= ' <a href="prime_edit_question.php?subject_module_id='.$subject_module_id.'&amp;question_subject_id='.$subject_id.'&amp;question_id='.$m['question_id'].'" title="'.$l['t_questions_editor'].' [ID = '.$m['question_id'].']" class="xmlbutton">'.$l['w_edit'].'</a>';

			$questlist .= '<br /><br />'.K_NEWLINE;
			$questlist .=  '<div class="paddingleft">'.F_decode_primecode($m['question_description']).'</div>'.K_NEWLINE;
			if (K_ENABLE_QUESTION_EXPLANATION AND !empty($m['question_explanation'])) {
				$questlist .=  '<div class="paddingleft"><br /><span class="explanation">'.$l['w_explanation'].':</span><br />'.F_decode_primecode($m['question_explanation']).'</div>'.K_NEWLINE;
			}
			if (!$hide_answers) {
				...
				if ($ra = F_db_query($sqla, $db)) {
					...
					while($ma = F_db_fetch_array($ra)) {
						...
						$answlist .= ' <a href="prime_edit_answer.php?subject_module_id='.$subject_module_id.'&amp;question_subject_id='.$subject_id.'&amp;answer_question_id='.$m['question_id'].'&amp;answer_id='.$ma['answer_id'].'" title="'.$l['t_answers_editor'].' [ID = '.$ma['answer_id'].']" class="xmlbutton">'.$l['w_edit'].'</a>';
						//$answlist .= " ";
						//$answlist .= "".F_decode_primecode($ma['answer_description'])."";
						$answlist .= '<br /><br />'.K_NEWLINE;
						$answlist .= '<div class="paddingleft">'.F_decode_primecode($ma['answer_description']).'</div>'.K_NEWLINE;
						if (K_ENABLE_ANSWER_EXPLANATION AND !empty($ma['answer_explanation'])) {
							$answlist .=  '<div class="paddingleft"><br /><span class="explanation">'.$l['w_explanation'].':</span><br />'.F_decode_primecode($ma['answer_explanation']).'</div>'.K_NEWLINE;
						}
