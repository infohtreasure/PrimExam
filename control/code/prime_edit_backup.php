<?php
//============================================================+
// File name   : prime_edit_backup.php
// Begin       : 2009-04-06
// Last Update : 2014-01-27
//
// Description : Backup and Restore primexam Database.
...
/**
 * @file
 * Backup and Restore primexam Database (works only on POSIX systems with MySQL or PostgreSQL).
		...
		if($forcerestore == $l['w_restore']) { //check if delete button has been pushed (redundant check)
			if (isset($backup_file) AND !empty($backup_file)) {
				...
				switch (K_DATABASE_TYPE) {
					case 'POSTGRESQL': {
						$filename = K_PATH_BACKUP.date('YmdHis').'_primexam_backup.tar';
					...
					case 'MYSQL':
					default: {
						$filename = K_PATH_BACKUP.date('YmdHis').'_primexam_backup.sql';
