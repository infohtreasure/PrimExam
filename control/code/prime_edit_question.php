<?php
//============================================================+
// File name   : prime_edit_question.php
// Begin       : 2004-04-27
...
require_once('../code/prime_functions_primecode_editor.php');
...
for ($id = 0; $id < 2; ++$id) {
	if(isset($_POST['sendfile'.$id]) AND ($_FILES['userfile'.$id]['name'])) {
		require_once('../code/prime_functions_upload.php');
...
if (($subject_module_id <= 0) OR ($question_subject_id <= 0)) {
	...
	echo '<p><a href="prime_edit_subject.php" title="'.$l['t_subjects_editor'].'" class="xmlbutton">&lt; '.$l['t_subjects_editor'].'</a></p>'.K_NEWLINE;
...
if($r = F_db_query($sql, $db)) {
	...
	while($m = F_db_fetch_array($r)) {
		...
		echo ' '.htmlspecialchars(F_remove_primecode($m['subject_name']), ENT_NOQUOTES, $l['a_meta_charset']).'</option>'.K_NEWLINE;
		$countitem++;
	}
	if ($countitem == 1) {
		echo '<option value="0">&nbsp;</option>'.K_NEWLINE;
...
if($r = F_db_query($sql, $db)) {
	...
	while($m = F_db_fetch_array($r)) {
		...
		echo ' '.htmlspecialchars(F_substr_utf8(F_remove_primecode($m['question_description']), 0, K_SELECT_SUBSTRING), ENT_NOQUOTES, $l['a_meta_charset']).'</option>'.K_NEWLINE;
		$countitem++;
	}
	if ($countitem == 1) {
		echo '<option value="0">&nbsp;</option>'.K_NEWLINE;
...
echo '<br />'.K_NEWLINE;
echo '<a href="#" title="'.$l['h_preview'].'" class="xmlbutton" onclick="previewWindow=window.open(\'prime_preview_primecode.php?primexamcode=\'+encodeURIComponent(document.getElementById(\'form_questioneditor\').question_description.value),\'previewWindow\',\'dependent,height=500,width=500,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no\'); return false;">'.$l['w_preview'].'</a>'.K_NEWLINE;
...
echo '<br />'.K_NEWLINE;
echo primecodeEditorTagButtons('form_questioneditor', 'question_description');
echo '</span>'.K_NEWLINE;
echo '</div>'.K_NEWLINE;
...
if (K_ENABLE_QUESTION_EXPLANATION) {
	...
	echo '<a href="#" title="'.$l['h_preview'].'" class="xmlbutton" onclick="previewWindow=window.open(\'prime_preview_primecode.php?primexamcode=\'+encodeURIComponent(document.getElementById(\'form_questioneditor\').question_explanation.value),\'previewWindow\',\'dependent,height=500,width=500,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no\'); return false;">'.$l['w_preview'].'</a>'.K_NEWLINE;
	...
	echo '<br />'.K_NEWLINE;
	echo primecodeEditorTagButtons('form_questioneditor', 'question_explanation');
	echo '</span>'.K_NEWLINE;
	echo '</div>'.K_NEWLINE;
}
...
if (isset($question_subject_id) AND ($question_subject_id > 0)) {
	echo '<a href="prime_edit_subject.php?subject_module_id='.$subject_module_id.'&amp;subject_id='.$question_subject_id.'" title="'.$l['t_subjects_editor'].'" class="xmlbutton">&lt; '.$l['t_subjects_editor'].'</a>';
}
echo '</span>'.K_NEWLINE;
echo '<span class="right">'.K_NEWLINE;
if (isset($question_id) AND ($question_id > 0)) {
	echo '<a href="prime_edit_answer.php?subject_module_id='.$subject_module_id.'&amp;question_subject_id='.$question_subject_id.'&amp;answer_question_id='.$question_id.'" title="'.$l['t_answers_editor'].'" class="xmlbutton">'.$l['t_answers_editor'].' &gt;</a>';
}
...
echo F_decode_primecode($question_description);
