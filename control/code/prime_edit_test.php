...
$thispage_title = $l['t_exams_editor'];
...
echo getFormRowTextInput('new_test_password', $l['w_password'], $l['h_test_password'], ' ('.$l['d_password_lenght'].')', '', '^([a-zA-Z0-9]{4,32})$', 255, false, false, true);
...
if (isset($test_id) AND ($test_id > 0)) {
	...
	if ($r = F_db_query($sql, $db)) {
		while ($m = F_db_fetch_array($r)) {
			...
			if ($rs = F_db_query($sqls, $db)) {
				while ($ms = F_db_fetch_array($rs)) {
					$subjects_list .= '<a href="prime_edit_subject.php?subject_id='.$ms['subject_id'].'" title="'.$l['t_subjects_editor'].'">'.htmlspecialchars($ms['subject_name'], ENT_NOQUOTES, $l['a_meta_charset']).'</a>, ';
				}
			} else {
				F_display_db_error();
			}
	...
	if (isset($test_max_score_new) and ($test_max_score_new > 0)) {
		...
		echo '<a href="prime_pdf_testgen.php?test_id='.$test_id.'&amp;num='.$test_num.'" title="'.$l['h_pdf_offline_test'].'" class="xmlbutton" onclick="pdfWindow=window.open(\'prime_pdf_testgen.php?test_id='.$test_id.'&amp;num=\' + document.getElementById(\'form_testeditor\').test_num.value + \'\',\'pdfWindow\',\'dependent,menubar=yes,resizable=yes,scrollbars=yes,status=yes,toolbar=yes\'); return false;">'.$l['w_generate'].'</a>';
		echo '</span>&nbsp;'.K_NEWLINE;
		echo '</div>'.K_NEWLINE;
	}
}
