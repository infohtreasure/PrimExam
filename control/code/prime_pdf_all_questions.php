<?php
//============================================================+
// File name   : prime_pdf_all_questions.php
// Begin       : 2004-06-10
...
switch ($expmode) {
	case 1: {
		...
		$pdf->setprimexamBackLink(K_PATH_URL.'control/code/prime_show_all_questions.php?subject_module_id='.$module_id.'&subject_id='.$subject_id);
		break;
	}
	case 2: {
		// Set backlink QR-Code
		$pdf->setprimexamBackLink(K_PATH_URL.'control/code/prime_show_all_questions.php?subject_module_id='.$module_id);
		break;
	}
	case 3: {
		// Set backlink QR-Code
		$pdf->setprimexamBackLink(K_PATH_URL.'control/code/prime_show_all_questions.php');
		break;
	}
}
	...
	while($mm = F_db_fetch_array($rm)) {
		...
		if($rs = F_db_query($sqls, $db)) {
			while($ms = F_db_fetch_array($rs)) {
				...
				$subject_description = F_decode_primecode($ms['subject_description']);
				...
				if($rq = F_db_query($sqlq, $db)) {
					...
					while($mq = F_db_fetch_array($rq)) {
						...
						$pdf->Ln();
						$pdf->writeHTMLCell(0, $data_cell_height, (PDF_MARGIN_LEFT + $data_cell_width_third), $pdf->GetY(), F_decode_primecode($mq['question_description']), 1, 1, '', '');
						if (K_ENABLE_QUESTION_EXPLANATION AND !empty($mq['question_explanation'])) {
							...
							$pdf->SetFont('', '');
							$pdf->writeHTMLCell(0, $data_cell_height, (PDF_MARGIN_LEFT + $data_cell_width_third), $pdf->GetY(), F_decode_primecode($mq['question_explanation']), 'LRB', 1, '', '');
						}
						...
						if ($show_answers) {
							...
							if($ra = F_db_query($sqla, $db)) {
								...
								while($ma = F_db_fetch_array($ra)) {
									...
									$pdf->Ln();
									$pdf->writeHTMLCell(0, $data_cell_height, (PDF_MARGIN_LEFT + (2 * $data_cell_width_third)), $pdf->GetY(), F_decode_primecode($ma['answer_description']), 1, 1, '', '', '');
									if (K_ENABLE_ANSWER_EXPLANATION AND !empty($ma['answer_explanation'])) {
										$pdf->Cell((2 * $data_cell_width_third), $data_cell_height, '', 0, 0, 'C', 0);
										$pdf->SetFont('', 'BIU');
										$pdf->Cell(0, $data_cell_height, $l['w_explanation'], 'LTR', 1, '', 0, '', 0);
										$pdf->SetFont('', '');
										$pdf->writeHTMLCell(0, $data_cell_height, (PDF_MARGIN_LEFT + (2 * $data_cell_width_third)), $pdf->GetY(), F_decode_primecode($ma['answer_explanation']), 'LRB', 1, '', '');
									}
								}
							} else {
								F_display_db_error();
...
switch ($expmode) {
	case 1: {
		$pdf_filename = 'primexam_subject_'.$subject_id.'_'.date('YmdHi').'.pdf';
		break;
	}
	case 2: {
		$pdf_filename = 'primexam_module_'.$module_id.'_'.date('YmdHi').'.pdf';
		break;
	}
	case 3: {
		$pdf_filename = 'primexam_all_modules_'.date('YmdHi').'.pdf';
		break;
	}
	default: {
		$pdf_filename = 'primexam_export_'.date('YmdHi').'.pdf';
		break;
	}
}
