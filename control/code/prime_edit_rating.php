<?php
//============================================================+
// File name   : prime_edit_rating.php
// Begin       : 2004-06-09
...
if($sql) {
	if($r = F_db_query($sql, $db)) {
		if($m = F_db_fetch_array($r)) {
			...
			$question = F_decode_primecode($m['question_description']);
			$explanation =  F_decode_primecode($m['question_explanation']);
			$answer = F_decode_primecode($m['testlog_answer_text']);
		} else {
			$testlog_id = '';
			$testlog_score = '';
			$test_score_right = 1;
