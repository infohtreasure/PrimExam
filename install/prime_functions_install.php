<?php
//============================================================+
// File name   : prime_functions_install.php
// Begin       : 2002-05-13
// Last Update : 2013-10-23
//
// Description : Installation functions for primexam.
//
...
function F_install_database($db_type, $db_host, $db_port, $db_user, $db_password, $database_name, $table_prefix, $drop_database, $create_new, $progress_log) {
	...
	if ($db = F_create_database(K_DATABASE_TYPE, $db_host, $db_port, $db_user, $db_password, $database_name, $table_prefix, $drop_database, $create_new)) { //create database if not exist
		...
		if (F_execute_sql_queries($db, strtolower(K_DATABASE_TYPE).'_db_structure.sql', 'prime_', $table_prefix, $progress_log)) {
			echo '<span style="color:#008000">[OK]</span></li>';
			...
			if (F_execute_sql_queries($db, 'db_data.sql', 'prime_', $table_prefix, $progress_log)) {
				echo '<span style="color:#008000">[OK]</span></li>';
 ...
 * Oracle databases must be created manually (create the primexam user and set the database name equal to user name)
	...
	if ($drop OR $create) {
		if ($db = @F_db_connect($host, $port, $user, $password)) {
			if ($dbtype == 'ORACLE') {
				...
				//       so you have to create a primexam user before calling this.
			} else {
				if ($drop) {
 ...
 * @param string $path_primexam relative URL where this program is installed
 ...
 * @return boolean true in case of success, false otherwise
 */
function F_update_config_files($db_type, $db_host, $db_port, $db_user, $db_password, $database_name, $table_prefix, $path_host, $path_primexam, $path_main, $standard_port, $progress_log) {
	...
	rename('../control/config.default', '../control/config');
	rename('../exam/config.default', '../exam/config');

	$config_file = array(); // configuration files

	$config_file[0] = '../shared/config/prime_db_config.php';
	$config_file[1] = '../shared/config/prime_paths.php';
	...
	$parameter[1] = array(
		...
		'1'  => array ('0' => "K_PATH_primeXAM', '([^\']*)'", '1' => "K_PATH_primeXAM', '".$path_primexam."'"),
